...
webconfig_subdoc_object_t   radio_stats_objects[3] = {
    { webconfig_subdoc_object_type_version, "Version" },
    { webconfig_subdoc_object_type_subdoc, "SubDocName" },
    { webconfig_subdoc_object_type_stats_config, "RadioChannelStats" },
};

webconfig_error_t init_radio_channel_stats_subdoc(webconfig_subdoc_t *doc)
{
    doc->num_objects = sizeof(radio_stats_objects)/sizeof(webconfig_subdoc_object_t);
    memcpy((unsigned char *)doc->objects, (unsigned char *)&radio_stats_objects, sizeof(radio_stats_objects));
...
webconfig_error_t encode_radio_channel_stats_subdoc(webconfig_t *config, webconfig_subdoc_data_t *data)
{
    ...
    cJSON_AddStringToObject(json, "SubDocName", "Radio_Channel_Stats");

    wifi_provider_response_t *chan_stats = params->collect_stats.stats;

    response_time = chan_stats->response_time;
    ...
    cJSON_AddNumberToObject(json, "RadioIndex", chan_stats->args.radio_index);

    scan_mode_enum = chan_stats->args.scan_mode;
    ...
    cJSON_AddItemToObject(json, "RadioChannelStats", st_obj_arr);

    if (encode_radio_channel_radio_params(chan_stats, st_obj_arr) != webconfig_error_none) {
...
webconfig_error_t decode_radio_channel_stats_subdoc(webconfig_t *config, webconfig_subdoc_data_t *data)
{
    ...
    // decode radio stats
    if (decode_radio_channel_radio_stats_object(ch_st, json) != webconfig_error_none) {
