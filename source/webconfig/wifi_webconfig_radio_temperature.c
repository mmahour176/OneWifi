...
webconfig_subdoc_object_t   radio_temperature_stats_objects[3] = {
    { webconfig_subdoc_object_type_version, "Version" },
    { webconfig_subdoc_object_type_subdoc, "SubDocName" },
    { webconfig_subdoc_object_type_stats_config, "RadioTemperatureStats" },
};

webconfig_error_t init_radio_temperature_stats_subdoc(webconfig_subdoc_t *doc)
{
    doc->num_objects = sizeof(radio_temperature_stats_objects)/sizeof(webconfig_subdoc_object_t);
    memcpy((unsigned char *)doc->objects, (unsigned char *)&radio_temperature_stats_objects, sizeof(radio_temperature_stats_objects));
...
webconfig_error_t encode_radio_temperature_stats_subdoc(webconfig_t *config, webconfig_subdoc_data_t *data)
{
    ...
    cJSON_AddStringToObject(json, "SubDocName", "Radio_Temperature_Stats");

    wifi_provider_response_t *radiotemperature_stats = params->collect_stats.stats;

    response_time = radiotemperature_stats->response_time;
    ...
    cJSON_AddNumberToObject(json, "RadioIndex", radiotemperature_stats->args.radio_index);

    // encode stats config objects
    st_obj_arr = cJSON_CreateArray();
    cJSON_AddItemToObject(json, "RadioTemperatureStats", st_obj_arr);

    if (encode_radio_temperature_params(radiotemperature_stats, st_obj_arr) != webconfig_error_none) {
...
webconfig_error_t decode_radio_temperature_stats_subdoc(webconfig_t *config, webconfig_subdoc_data_t *data)
{
    ...
    wifi_provider_response_t **temp_st = (wifi_provider_response_t **)&params->collect_stats.stats;
