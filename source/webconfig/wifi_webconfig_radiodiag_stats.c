...
webconfig_subdoc_object_t   radiodiag_stats_objects[3] = {
    { webconfig_subdoc_object_type_version, "Version" },
    { webconfig_subdoc_object_type_subdoc, "SubDocName" },
    { webconfig_subdoc_object_type_stats_config, "RadioDiagnosticStats" },
};

webconfig_error_t init_radiodiag_stats_subdoc(webconfig_subdoc_t *doc)
{
    doc->num_objects = sizeof(radiodiag_stats_objects)/sizeof(webconfig_subdoc_object_t);
    memcpy((unsigned char *)doc->objects, (unsigned char *)&radiodiag_stats_objects, sizeof(radiodiag_stats_objects));
...
webconfig_error_t encode_radio_radiodiag_stats_subdoc(webconfig_t *config, webconfig_subdoc_data_t *data)
{
    ...
    cJSON_AddStringToObject(json, "SubDocName", "Radio_Diagnostics_Stats");

    wifi_provider_response_t *radiodiag_stats = params->collect_stats.stats;

    response_time = radiodiag_stats->response_time;
    ...
    cJSON_AddNumberToObject(json, "RadioIndex", radiodiag_stats->args.radio_index);

    // encode stats config objects
    st_obj_arr = cJSON_CreateArray();
    cJSON_AddItemToObject(json, "RadioDiagnosticStats", st_obj_arr);

    if (encode_radiodiag_params(radiodiag_stats, st_obj_arr) != webconfig_error_none) {
...
webconfig_error_t decode_radio_radiodiag_stats_subdoc(webconfig_t *config, webconfig_subdoc_data_t *data)
{
    ...
    wifi_provider_response_t **diag_st = (wifi_provider_response_t **)&params->collect_stats.stats;
